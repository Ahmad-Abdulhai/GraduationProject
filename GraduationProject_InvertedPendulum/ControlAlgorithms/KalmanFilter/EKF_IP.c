...
double x_k_1, o_k_1, v_k_1, w_k_1, Uv, x_m;

// Cumputation Matrixes
double tsam; // constatnts
//    double  Ad[4][3],Bd[3][1],C[1][3],
double Pk_k[4][4],Q[4][4],R[1][1];
double fx_k[4][4],fu_k[4][1],Pk_1_k[4][4],fx_k_T[4][4],fu_k_T[1][4];
double K_g[4][1],H[1][4],H_T[4][1];
double Pk_H_T[4][1]; //P(k+1)H'
double H_Pk_H_T[1][1];//HP(k+1)H'
double fx_P[4][4], fu_q[4][1],PK_1_K_1[4][4];   //JP(k)
double fx_P_fx[4][4], fu_q_fu[4][4], HPH_R[1][1]; //JP(k)J'
...
int    i,j,k;
double  mc, mpw, mps, l, fc , fp, J, g, vf, mp, a, mu, error, sum;
...
Pk_k[0][0]=1e-3;///100;  //  related to ia
Pk_k[0][1]=0.0;
Pk_k[0][2]=0.0;
Pk_k[0][3]=0.0;

Pk_k[1][0]=0.0;
Pk_k[1][1]=1e-3;////100;       related to Wr
Pk_k[1][2]=0.0;
Pk_k[1][3]=0.0;
...
Q[0][0]=1000; Q[0][1]=0; Q[0][2]=0; Q[0][3]=0;
Q[1][0]=0; Q[1][1]=50; Q[1][2]=0; Q[1][3]=0;
Q[2][0]=0; Q[2][1]=0; Q[2][2]=50; Q[2][3]=0;
Q[3][0]=0; Q[3][1]=0; Q[3][2]=0; Q[3][3]=50;
...
fx_k[1][3] = tsam;
fx_k[2][0] = 0;
fx_k[2][1] = tsam*((-a*mu*cos(o_k_1)*pow(w_k_1,2) +fp*l*sin(o_k_1)*w_k_1 +g*l*mu*(2*pow(cos(o_k_1),2) -1))*J +(2*l*mu*(sin(2*o_k_1)/2)*(a*(mu*sin(o_k_1)*pow(w_k_1,2) + v_k_1*fc -Uv*vf) +l*cos(o_k_1)*(w_k_1*fp -g*mu*sin(o_k_1)))))/pow(J,2);
fx_k[2][2] =  1 -(tsam*a*fc)/J; // 0.9978;
fx_k[2][3] = -(tsam*(fp*l*cos(o_k_1) +2*a*w_k_1*mu*sin(o_k_1)))/J;
fx_k[3][0] = 0;
fx_k[3][1] = tsam*((l*sin(o_k_1)*(mu*sin(o_k_1)*pow(w_k_1,2) +v_k_1*fc -Uv*vf) +g*mu*cos(o_k_1) -pow(w_k_1,2)*l*mu*pow(cos(o_k_1),2))/J +(2*l*mu*(sin(2*o_k_1)/2)*(w_k_1*fp -g*mu*sin(o_k_1) +l*cos(o_k_1)*(mu*sin(o_k_1)*pow(w_k_1,2) +v_k_1*fc -Uv*vf)))/pow(J,2));
fx_k[3][2] = -(tsam*fc*l*cos(o_k_1))/J; // -0.0060*cos(o_k_1); 
fx_k[3][3] = 1 - (tsam*(fp + 2*w_k_1*l*mu*(sin(2*o_k_1)/2)))/J;
...
////////////////Kalman Filter Gain Calculation//////////
    ...
    
    {
        ...
        for (i=0;i<1;i++){
        for (j=0;j<1;j++){
            sum=sum+Pk_H_T[k][j]/HPH_R[j][i]; // (P(-)*H(k)')/(H(k)*P(-)*H(k)' + R)
                         }
            K_g[k][i]=sum;
            sum=0.0;
                         }
...
x_k_1 = x_es_k[0]+K_g[0][0]*error; // x_k_1
o_k_1 = x_es_k[1]+K_g[1][0]*error; // o_k_1
v_k_1 = x_es_k[2]+K_g[2][0]*error; // v_k_1
w_k_1 = x_es_k[3]+K_g[3][0]*error; // w_k_1
