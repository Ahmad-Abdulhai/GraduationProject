...
#include <string.h>
#include "IPtest1.h"
#include "IPtest1_private.h"
#include "ext_work.h"

#include "IPtest1_dt.h"
#include "simstruc.h"
...
SimStruct *IPtest1(void)
{
  ...
  ssSetModelName(rtS, "IPtest1");
  ssSetPath(rtS, "IPtest1");
  ...
  ssSetChecksumVal(rtS, 0, 3414765607U);
  ssSetChecksumVal(rtS, 1, 2458611456U);
  ssSetChecksumVal(rtS, 2, 448705726U);
  ssSetChecksumVal(rtS, 3, 3908556449U);
  ...
  /* register each child */
  {
    ...
    /* Level2 S-Function Block: IPtest1/<Root>/Read Cart Encoder1 (pci1711FedEnc) */
    {
      SimStruct *rts = ssGetSFunction(rtS, 0);
      /* timing info */
      ...
      ssSetPath(rts, "IPtest1/Read Cart Encoder1");
    ...
    /* Level2 S-Function Block: IPtest1/<Root>/Read Pendulum Encoder (pci1711FedEnc) */
    {
      SimStruct *rts = ssGetSFunction(rtS, 1);
      /* timing info */
      static time_T sfcnPeriod[1];
      ...
      ssSetPath(rts, "IPtest1/Read Pendulum Encoder");
      if (ssGetRTModel(rtS) == NULL) {
        ssSetParentSS(rts, rtS);
        ssSetRootSS(rts, ssGetRootSS(rtS));
      } else {
