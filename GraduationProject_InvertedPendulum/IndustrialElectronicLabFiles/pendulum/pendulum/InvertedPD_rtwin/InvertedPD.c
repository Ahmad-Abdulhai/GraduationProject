...
void MdlOutputs(int_T tid)
{
  ...
  if (ssIsContinuousTask(rtS, tid)) {   /* Sample time: [0.0, 0.0] */
     ...
     *   Expression: -u[3]
     */
    rtb_temp20 = ( -rtb_Sum1[2] );

    /* Signum: '<S4>/Sign2' */
    rtb_Sign2 = rt_SGN(rtb_temp20);
  }
  ...
  if (ssIsContinuousTask(rtS, tid)) {   /* Sample time: [0.0, 0.0] */
     ...
     *
     * Regarding '<S6>/Fcn3':
     *   Expression: K2*atan2(sin(u[2]),cos(u[2]))
     *
     * Regarding '<S6>/Fcn2':
     *   Expression: K4*u[4]
     *
     ...
     */
    rtb_temp20 =
      ((130.0 * rt_atan2( sin( rtb_Sum1[1] ) , cos( rtb_Sum1[1] ) ) ) + (35.0 *
      rtb_Sum1[3] ))
      + 0.0 + (((-40.0) * rtb_Sum1[0] ) + ((-80.0) * rtb_Sum1[2] ));
     ...
     */
    if (rtb_temp20 >= rtP.Saturation_UpperSat) {
      rtB.Saturation = rtP.Saturation_UpperSat;
    } else if (rtb_temp20 <= rtP.Saturation_LowerSat) {
      rtB.Saturation = rtP.Saturation_LowerSat;
    } else {
      rtB.Saturation = rtb_temp20;
    }
  }
}
...
void MdlInitializeSizes(void)
{
  ...
  ssSetNumBlocks(rtS, 34);              /* Number of blocks */
...
SimStruct *InvertedPD(void)
{
  ...
  ssSetModelName(rtS, "InvertedPD");
  ssSetPath(rtS, "InvertedPD");
  ...
  ssSetChecksumVal(rtS, 0, 393556U);
  ssSetChecksumVal(rtS, 1, 3382365537U);
  ssSetChecksumVal(rtS, 2, 3525418037U);
  ssSetChecksumVal(rtS, 3, 3911799000U);
  ...
  /* register each child */
  {
    ...
    /* Level2 S-Function Block: InvertedPD/<Root>/Digital To Analog Converter (pci1711FedDac) */
    {
      SimStruct *rts = ssGetSFunction(rtS, 0);
      /* timing info */
      ...
      ssSetPath(rts, "InvertedPD/Digital To Analog Converter");
    ...
    /* Level2 S-Function Block: InvertedPD/<Root>/Read Cart Encoder1 (pci1711FedEnc) */
    {
      SimStruct *rts = ssGetSFunction(rtS, 1);
      /* timing info */
      static time_T sfcnPeriod[1];
      ...
      ssSetPath(rts, "InvertedPD/Read Cart Encoder1");
      if (ssGetRTModel(rtS) == NULL) {
        ssSetParentSS(rts, rtS);
        ssSetRootSS(rts, ssGetRootSS(rtS));
      } else {
    ...
    /* Level2 S-Function Block: InvertedPD/<Root>/Read Pendulum Encoder (pci1711FedEnc) */
    {
      SimStruct *rts = ssGetSFunction(rtS, 2);
      /* timing info */
      static time_T sfcnPeriod[1];
      ...
      ssSetPath(rts, "InvertedPD/Read Pendulum Encoder");
      if (ssGetRTModel(rtS) == NULL) {
        ssSetParentSS(rts, rtS);
        ssSetRootSS(rts, ssGetRootSS(rtS));
      } else {
