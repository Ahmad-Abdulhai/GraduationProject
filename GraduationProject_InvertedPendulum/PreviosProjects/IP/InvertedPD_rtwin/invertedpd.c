...
void MdlStart(void)
{
  ...
  rtB.Desired_Position = rtP.Constant_Value;
...
void MdlOutputs(int_T tid)
{
  ...
  if (ssIsSampleHit(rtS, 1, tid)) {     /* Sample time: [0.001, 0.0] */
     ...
     */
    rtB.Gain1 = rtB.Cart_Position * rtP.Gain1_Gain;
     ...
     */
    rtB.Gain3 = rtB.Pendulum_Position * rtP.Gain3_Gain;

    /* Gain: '<S1>/N'
     *
     * Regarding '<S1>/N':
     *   Gain value: rtP.N_Gain
     */
    rtB.N = rtB.Desired_Position * rtP.N_Gain;
  }
  ...
  if (ssIsContinuousTask(rtS, tid)) {   /* Sample time: [0.0, 0.0] */
     ...
     */
    rtb_actual_LoadPosition = (-rtB.Gain1 - rtB.Gain3 - (rtB.Cart_Velocity *
      rtP.Gain_Gain) - (rtB.Pendulum_Velocity * rtP.Gain2_Gain) + rtB.N) *
      rtP.Scaling_Gain;
...
void MdlInitializeSizes(void)
{
  ...
  ssSetNumBlocks(rtS, 19);              /* Number of blocks */
...
SimStruct *invertedpd(void)
{
  ...
  ssSetChecksumVal(rtS, 0, 761398706U);
  ssSetChecksumVal(rtS, 1, 578454909U);
  ssSetChecksumVal(rtS, 2, 2145052380U);
  ssSetChecksumVal(rtS, 3, 2719397872U);
