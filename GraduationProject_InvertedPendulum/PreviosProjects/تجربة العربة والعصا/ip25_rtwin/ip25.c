/*
 * Real-Time Workshop code generation for Simulink model "ip25.mdl".
 *
 * Model Version                        : 1.87
 * Real-Time Workshop file version      : 5.0 $Date: 2002/05/30 19:21:33 $
 * Real-Time Workshop file generated on : Thu Jan 01 00:05:03 2004
 * TLC version                          : 5.0 (Jun 18 2002)
 * C source code generated on           : Thu Jan 01 00:05:03 2004
 */

#include <math.h>
#include <string.h>
#include "ip25.h"
#include "ip25_private.h"
#include "ext_work.h"
...
void MdlStart(void)
{
  ...
  /* FromWorkspace Block: <S3>/FromWs */
  {

    static real_T pTimeValues[] = { 0.0, 19.8, 19.8, 40.0, 40.0, 120.0 };

    static real_T pDataValues[] = { 0.0, 0.0, 0.2, 0.2, -0.2, -0.2 };

    rtDWork.FromWs_PWORK.TimePtr = (void *) pTimeValues;
    rtDWork.FromWs_PWORK.DataPtr = (void *) pDataValues;

    rtDWork.FromWs_IWORK.PrevIndex = 0;
  }
...
void MdlOutputs(int_T tid)
{
  ...
  if (ssIsContinuousTask(rtS, tid)) {   /* Sample time: [0.0, 0.0] */
    ...
    {
      real_T *pDataValues = (real_T *) rtDWork.FromWs_PWORK.DataPtr;
      real_T *pTimeValues = (real_T *) rtDWork.FromWs_PWORK.TimePtr;
      int_T currTimeIndex = rtDWork.FromWs_IWORK.PrevIndex;
      ...
      rtDWork.FromWs_IWORK.PrevIndex = currTimeIndex;
      ...
      /* post output */
      {
        ...
        if (t1 == t2) {
          if (t < t1) {
            rtB.FromWs = pDataValues[currTimeIndex];
          } else {
            rtB.FromWs = pDataValues[currTimeIndex + 1];
          }
        } else {
          ...
          rtB.FromWs = (real_T) rtInterpolate(d1, d2, f1, f2);
          pDataValues += 6;
        }
      }
    }
  ...
  if (ssIsContinuousTask(rtS, tid)) {   /* Sample time: [0.0, 0.0] */
     ...
     */
    rtb_Sum1[0] = - rtB.Cart_Position + rtB.FromWs;
...
void MdlInitializeSizes(void)
{
  ...
  ssSetNumBlockParams(rtS, 52);         /* Sum of parameter "widths" */
}
...
SimStruct *ip25(void)
{
  ...
  ssSetChecksumVal(rtS, 0, 1334438422U);
  ssSetChecksumVal(rtS, 1, 616783888U);
  ssSetChecksumVal(rtS, 2, 3891121206U);
  ssSetChecksumVal(rtS, 3, 1601201137U);
  ...
  /* register each child */
  {
    ...
    /* Level2 S-Function Block: ip25/<Root>/Digital To Analog Converter (pci1711FedDac) */
    {
      SimStruct *rts = ssGetSFunction(rtS, 0);
      /* timing info */
      ...
      ssSetPath(rts, "ip25/Digital To Analog Converter");
    ...
    /* Level2 S-Function Block: ip25/<Root>/Read Cart Encoder1 (pci1711FedEnc) */
    {
      SimStruct *rts = ssGetSFunction(rtS, 1);
      /* timing info */
      static time_T sfcnPeriod[1];
      ...
      ssSetPath(rts, "ip25/Read Cart Encoder1");
      if (ssGetRTModel(rtS) == NULL) {
        ssSetParentSS(rts, rtS);
        ssSetRootSS(rts, ssGetRootSS(rtS));
      } else {
    ...
    /* Level2 S-Function Block: ip25/<Root>/Read Pendulum Encoder (pci1711FedEnc) */
    {
      SimStruct *rts = ssGetSFunction(rtS, 2);
      /* timing info */
      static time_T sfcnPeriod[1];
      ...
      ssSetPath(rts, "ip25/Read Pendulum Encoder");
      if (ssGetRTModel(rtS) == NULL) {
        ssSetParentSS(rts, rtS);
        ssSetRootSS(rts, ssGetRootSS(rtS));
      } else {
